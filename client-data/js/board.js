...
Tools.HTML = {
  ...
  removeStylesheet: function (href) {
...
Tools.registerWithStyles = function (newTool) {
  if (Tools.isBlocked(newTool)) return;
...
Tools.enableTool = function (tool) {
  if (Tools.isBlocked(tool)) return;
...
Tools.disableTool = function (tool) {
  if (tool.stylesheet) {
    Tools.HTML.removeStylesheet(tool.stylesheet);
  }
  const id = "toolID-" + tool.name;
  const toolInDom = document.getElementById(id);
  toolInDom.remove();
  Tools.change("Hand");
};
...
Tools.homeButton = document.getElementById("home-button");
Tools.homeButton.onclick = () => {
  window.location.href = "/";
};
